...
app.post("/debug/fix-fees-once", async (req, res) => {
  try {
    try { await fs.access(MIGRATION_SENTINEL); return res.json({ ok: true, alreadyRun: true }); } catch {}
    await loadData();
    ...
    res.json({ ok: true, changed, total: purchases.length });
  } catch (e) {
    console.error("fix-fees-once error:", e);
