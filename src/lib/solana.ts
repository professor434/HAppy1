...
// ==== Extrnode endpoints (HTTPS + WSS) ====
const EXTR = "solana-mainnet.rpc.extrnode.com/abba3bc7-b46a-4acb-8b15-834781a11ae2";
const HTTP = `https://${EXTR}`;
const WS   = `wss://${EXTR}`;

export const connection = new Connection(HTTP, { commitment: "confirmed", wsEndpoint: WS, confirmTransactionInitialTimeout: 90_000 });
...
// ==== Mobile-safe sender (prefers wallet.sendTransaction) ====
async function signAndSendTransaction(
  ...
  if (typeof (wallet as any).sendTransaction === "function") {
    ...
    const sig: TransactionSignature = await send(tx, connection, { skipPreflight: false, preflightCommitment: "confirmed", maxRetries: 3 });
