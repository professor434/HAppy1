...
const RPC_ENDPOINT = (ENV.VITE_SOLANA_RPC_URL as string) || DEFAULT_EXTRNODE;
...
export const BUY_FEE_PERCENTAGE = ENV.VITE_BUY_FEE_PERCENTAGE ? Number(ENV.VITE_BUY_FEE_PERCENTAGE) : 0.4;
...
export async function executeSOLPayment(amountSOL: number, wallet: any): Promise<TransactionSignature> {
...
export async function executeUSDCPayment(amountUSDC: number, wallet: any): Promise<TransactionSignature> {
  const feePct = BUY_FEE_PERCENTAGE / 100;
  ...
  catch { tx.add(createAssociatedTokenAccountInstruction(owner, toMain, TREASURY_WALLET, USDC_MINT_ADDRESS)); }
  try { await getAccount(connection, toFee); }
  catch { tx.add(createAssociatedTokenAccountInstruction(owner, toFee, FEE_WALLET, USDC_MINT_ADDRESS)); }
...
export async function executeClaimFeePayment(wallet: any): Promise<TransactionSignature> {
  const tx = new Transaction().add(
