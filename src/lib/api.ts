const RAW = (import.meta as any)?.env?.VITE_API_BASE_URL || "https://happy-pennis.up.railway.app";
export const API_BASE_URL = String(RAW).replace(/\/+$/, "");

// debug helper στο browser
// @ts-ignore
...
export type PurchaseRecord = {
  ...
  total_paid_usdc?: number; total_paid_sol?: number; fee_paid_usdc?: number; fee_paid_sol?: number;
...
export async function canClaimTokensBulk(wallets: string[]) {
  if (wallets.length === 1) {
    const w = wallets[0];
    const one = await tryAlt<any>(() => j(`/can-claim/${encodeURIComponent(w)}`),
      () => j<any[]>("/can-claim", { method: "POST", body: JSON.stringify({ wallets: [w] }) })
        .then(a => (a && a[0]) || { wallet: w, canClaim: false })
    );
