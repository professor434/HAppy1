...
export function assertEnv() {
  ...
  if (missing.length) {
    ...
    // Docs: import.meta.env & VITE_ prefix. :contentReference[oaicite:2]{index=2}
    console.warn(
      `[env] Λείπουν μεταβλητές: ${missing.join(", ")}. ` +
      `Θέλουν VITE_ prefix και ρύθμιση στα Vercel/Railway.`
    );
  }
}
